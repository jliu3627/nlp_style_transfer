...
def create_app(pipeline: Optional[ToxicityRewritePipeline] = None) -> FastAPI:
    ...
    def transform(request: TransformRequest) -> TransformResponse:
        ...
        active_pipeline = app.state.pipeline or _build_pipeline(request.toxicity_threshold)
        results = [item.to_dict() for item in active_pipeline.transform_many(texts)]
...
def _build_pipeline(toxicity_threshold: Optional[float]) -> ToxicityRewritePipeline:
    if toxicity_threshold is None:
        threshold = float(os.getenv("TOXICITY_THRESHOLD", "0.5"))
    else:
        threshold = toxicity_threshold
    return ToxicityRewritePipeline(toxicity_threshold=threshold)
